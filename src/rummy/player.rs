        ...
        fn new()->Self{
            ...
            fn play_turn(hand:&mut Hand, topCard: &Card<PlayingCard>, deckCard: &Card<PlayingCard>, turnNum: &usize)-> (Card<PlayingCard>, bool, bool)
            {
                ...
                let mut drew_card = false;
                 if RummyUtils::in_something(&hand.current_hand, topCard)
                 {
                    hand.card_to_hand(*topCard);
                 }
                 else 
                 {
                    hand.card_to_hand(*deckCard);
                    drew_card = true;
                 }

                 // Mark how many matches each card has
                 RummyUtils::mark_safe(&mut hand.current_hand);
                 ...
                 for i in hand.current_hand.iter()
                 {
                ...
                // end of function
                 ...
                 println!("ratings: {:?}",ratings);
                                  let mut k = 0;
                for l in hand.current_hand.iter()
                {
                    println!("opponent: k{} {:?}",k,l);
                    k +=1;
                }
                // Save card to return 
                let discarded_card = hand.current_hand[max_index];

                // remove card from vectors
                ratings.remove(max_index);
                hand.card_from_hand(max_index);
...
impl Brain for UserP{
    ...
    fn play_turn(hand:&mut Hand, topCard: &Card<PlayingCard>, deckCard: &Card<PlayingCard>, turnNum: &usize)-> (Card<PlayingCard>, bool, bool)
    {
        RummyUtils::value_sort(&mut hand.current_hand);
         println!("Top discard: {:?}", topCard);
         println!("Top Draw: {:?}",deckCard);
        ...
        let discard_card = hand.current_hand[card_select];
        hand.card_from_hand(card_select);
